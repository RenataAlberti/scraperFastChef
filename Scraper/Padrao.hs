...
render ReceitasDeHoje q = "http://www.receitasdehoje.com.br/" `append` TE.decodeUtf8 (toByteString $ renderQueryText True (DL.map (second Just) q)) 
...
constructUrl ReceitasDeHoje x = unpack $ render ReceitasDeHoje [(pack "s", pack x)]
